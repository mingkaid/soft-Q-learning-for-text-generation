...
class StyleTransformer(nn.Module):
    ...
    def forward(self, inp_tokens, gold_tokens, inp_lengths, style,
                generate=False, generate_size=1, differentiable_decode=False, 
                temperature=1.0, top_k=50, top_p=1.0, sample=False):
        
        if generate:
            inp_tokens = inp_tokens.repeat(generate_size, 1)
            style = style.repeat(generate_size)
            ...
            for k in range(self.max_length):
                ...
                if differentiable_decode:
                    ...
                    if not sample:
                        ...
                        filtered_logits = top_k_top_p_filtering(log_prob, top_k=top_k, top_p=top_p).squeeze(1)
                        next_id = torch.multinomial(F.softmax(filtered_logits, dim=-1), num_samples=1)
                        
                    next_token = self.embed(next_id, pos_idx[:, k:k+1])
                    
                output_ids.append(next_id)
                log_probs.append(log_prob)
                
            output_ids = torch.cat(output_ids, 1)
            log_probs = torch.cat(log_probs, 1)
            self.attn_weights = attn_weight           
            return log_probs, output_ids
